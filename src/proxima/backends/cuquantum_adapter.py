...
class CuQuantumGPUError(CuQuantumError):
    ...
    def __init__(self, message: str, device_id: int = 0, gpu_info: GPUDeviceInfo | None = None):
        details = {
            "device_id": device_id,
...
class CuQuantumAdapter(BaseBackendAdapter):
    ...
    def get_version(self) -> str:
        ...
        try:
            import qiskit_aer
            versions.append(f"qiskit-aer={getattr(qiskit_aer, '__version__', 'unknown')}")
        except Exception:
            pass
    ...
    def get_capabilities(self) -> Capabilities:
        ...
        return Capabilities(
            ...
            custom_features={
                ...
                "compute_capability": self._gpu_devices[0].compute_capability if self._gpu_devices else None,
            },
        )
    ...
    def execute(self, circuit: Any, options: dict[str, Any] | None = None) -> ExecutionResult:
        ...
        if not validation.valid:
            if self._config.fallback_to_cpu:
                self._logger.warning(f"Circuit validation failed: {validation.message}. Falling back to CPU.")
                return self._execute_on_cpu(circuit, options)
        ...
        try:
            ...
            if self._config.fallback_to_cpu:
                self._logger.warning(f"GPU execution failed: {exc}. Falling back to CPU.")
                return self._execute_on_cpu(circuit, options)
    ...
    def _execute_on_gpu(
        ...
        device = self.get_gpu_info(options.get("gpu_device_id", self._config.gpu_device_id))
