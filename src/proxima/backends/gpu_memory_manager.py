...
class GPUMemoryManager:
    ...
    def query_current_gpu_memory(self, device_id: int | None = None) -> dict[str, float]:
        ...
        utilization = used_mb / gpu_info.total_memory_mb if gpu_info.total_memory_mb > 0 else 0
    ...
    def initialize_memory_pool(self, pool_size_mb: float | None = None) -> bool:
        ...
        with self._pool_lock:
            ...
            try:
                ...
                self._pool_stats.available_mb = pool_size_mb
                self._pool_initialized = True
                self._logger.info(f"Memory pool stats initialized: {pool_size_mb:.1f} MB")
                return True
    ...
    def allocate_from_pool(self, size_mb: float) -> tuple[bool, int | None]:
        ...
        with self._pool_lock:
            ...
            self._logger.debug(f"Allocated {size_mb:.1f} MB from pool (ID: {allocation_id})")
    ...
    def release_to_pool(self, allocation_id: int, size_mb: float) -> bool:
        ...
        with self._pool_lock:
            ...
            if total_ops > 0:
                self._pool_stats.cache_hit_rate = self._pool_stats.reuse_count / total_ops
            
            self._logger.debug(f"Released {size_mb:.1f} MB to pool (ID: {allocation_id})")
            return True
...
def estimate_gpu_memory_for_qubits(
    ...
    """
    prec = CuQuantumPrecision.SINGLE if precision == "single" else CuQuantumPrecision.DOUBLE
...
def get_recommended_precision_for_memory(
    ...
    Returns:
        "single" or "double"
    """
    double_estimate = estimate_gpu_memory_for_qubits(qubit_count, "double", workspace_mb)
    single_estimate = estimate_gpu_memory_for_qubits(qubit_count, "single", workspace_mb)
