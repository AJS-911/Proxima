    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if unsupported_gates:
            self.suggestions.insert(0, f"Unsupported gates: {', '.join(unsupported_gates)}")
            self.suggestions.append("Consider decomposing custom gates into basic gates")
...
class QuestBackendAdapter(BaseBackendAdapter):
    ...
    GATE_MAP: dict[str, str] = {
        ...
        "rx": "Rx", "ry": "Ry", "rz": "Rz",
        # Two-qubit gates
        "cx": "X", "cnot": "X",  # Controlled-X
        "cy": "Y", "cz": "Z",    # Controlled-Y/Z
        "swap": "SWAP",
    }
    
    # Supported gates list for validation
    SUPPORTED_GATES: set[str] = {
        "h", "hadamard", "x", "pauli_x", "not", "y", "pauli_y", "z", "pauli_z",
        "s", "t", "sdg", "tdg", "rx", "ry", "rz", "cx", "cnot", "cy", "cz",
        "swap", "ccx", "toffoli", "ccz", "cswap", "fredkin",
        "u1", "u2", "u3", "p", "phase", "id", "i", "identity",
        "sx", "sxdg", "iswap",
    }
    ...
    def _detect_gpu_details(self) -> None:
        ...
        try:
            ...
            self._hardware_info.cuda_version = f"{version // 1000}.{(version % 1000) // 10}"
        except ImportError:
            try:
                import cupy
                device = cupy.cuda.Device(self._config.gpu_device_id)
                self._hardware_info.gpu_device_count = cupy.cuda.runtime.getDeviceCount()
                props = cupy.cuda.runtime.getDeviceProperties(self._config.gpu_device_id)
                self._hardware_info.gpu_device_name = props["name"].decode() if isinstance(props["name"], bytes) else props["name"]
                self._hardware_info.gpu_memory_mb = props["totalGlobalMem"] // (1024 * 1024)
            except ImportError:
                pass
        except Exception as e:
    ...
    def get_capabilities(self) -> Capabilities:
        ...
        gpu_available = False
        if self._hardware_info:
            gpu_available = self._hardware_info.gpu_available and self._config.gpu_enabled
    ...
    def validate_circuit(self, circuit: Any) -> ValidationResult:
        ...
        try:
            ...
            return ValidationResult(
                valid=False, 
                message=f"pyQuEST import failed: {exc}"
            )
        ...
        if isinstance(circuit, list):
            ...
            for i, gate in enumerate(circuit):
                if not isinstance(gate, dict):
                    return ValidationResult(
                        valid=False, 
                        message=f"Gate {i} is not a dictionary"
                    )
                if "gate" not in gate:
                    return ValidationResult(
                        valid=False, 
                        message=f"Gate {i} missing 'gate' key"
                    )
                if "qubits" not in gate:
                    return ValidationResult(
                        valid=False, 
                        message=f"Gate {i} missing 'qubits' key"
                    )
    ...
    def estimate_resources(self, circuit: Any) -> ResourceEstimate:
        ...
        dm_memory_mb = self._estimate_memory_mb(qubits, True, self._config.precision) if qubits <= self.MAX_QUBITS_DM else None
        ...
        metadata = {
            ...
            "density_matrix_memory_mb": round(dm_memory_mb, 2) if dm_memory_mb else None,
            "gpu_available": self._hardware_info.gpu_available if self._hardware_info else False,
        }
    ...
    def execute(
        ...
        max_qubits = self.MAX_QUBITS_DM if sim_type == SimulatorType.DENSITY_MATRIX else self.MAX_QUBITS
        ...
        use_gpu = config.gpu_enabled and self._hardware_info and self._hardware_info.gpu_available
        ...
        try:
            ...
            if repetitions > 0:
                ...
                counts = self._sample_measurements(register, qubit_count, repetitions, pyQuEST, unitaries)
        ...
        except (QuestInstallationError, QuestCircuitError, QuestMemoryError, 
                QuestGPUError, QubitLimitExceededError, UnsupportedOperationError):
            raise
    ...
    def _map_cirq_gate(self, gate: Any, qubits: list[int], unitaries: Any) -> Any | None:
    ...
    def _convert_qiskit_circuit(self, circuit: Any, pyQuEST: Any, unitaries: Any) -> Any:
    ...
    def _apply_gates(self, register: Any, gates: list[dict], unitaries: Any) -> None:
        ...
        for gate in gates:
            ...
            if gate_name in ("h", "hadamard"):
                op = unitaries.H(target, controls=controls) if controls else unitaries.H(target)
            elif gate_name in ("x", "pauli_x", "not"):
                op = unitaries.X(target, controls=controls) if controls else unitaries.X(target)
            elif gate_name in ("y", "pauli_y"):
                op = unitaries.Y(target, controls=controls) if controls else unitaries.Y(target)
            elif gate_name in ("z", "pauli_z"):
                op = unitaries.Z(target, controls=controls) if controls else unitaries.Z(target)
                ...
                op = unitaries.Rx(target, angle, controls=controls) if controls else unitaries.Rx(target, angle)
            elif gate_name == "ry" and params:
                angle = float(params[0])
                op = unitaries.Ry(target, angle, controls=controls) if controls else unitaries.Ry(target, angle)
            elif gate_name == "rz" and params:
                angle = float(params[0])
                op = unitaries.Rz(target, angle, controls=controls) if controls else unitaries.Rz(target, angle)
            ...
            elif gate_name == "swap" and len(qubits) >= 2 and hasattr(unitaries, "SWAP"):
