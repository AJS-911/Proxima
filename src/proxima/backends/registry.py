...
class BackendComparisonMatrix:
    ...
    def to_markdown(self) -> str:
        ...
        for e in entries:
            ...
            gpu = "✓" if e.supports_gpu else "✗"
            noise = "✓" if e.supports_noise else "✗"
            mpi = "✓" if e.supports_mpi else "✗"
...
__all__ = [
    ...
    def verify_all_backends(self) -> RegistrationVerificationResult:
        """Verify all expected backends are registered."""
        return self._verifier.verify_all(self._statuses)

    def verify_backend(self, name: str) -> bool:
        """Verify a specific backend is registered and available."""
        return self._verifier.verify_backend(name, self._statuses)

    def get_missing_backends(self) -> list[str]:
        """Get list of expected backends that are missing."""
        return self._verifier.get_missing_backends(self._statuses)

    # ==================== HEALTH MONITORING FEATURES ====================

    def start_health_monitoring(self, interval: float = 60.0) -> None:
        """Start periodic health monitoring."""
    ...
    def check_backend_health(self, name: str) -> HealthCheckResult:
        ...
        with self._lock:
            status = self._statuses.get(name)
            if not status or not status.adapter:
                return HealthCheckResult(
                    backend_name=name,
                    is_healthy=False,
            ...
            result = self._health_monitor.check_health(name, status.adapter)
            status.health_status = result.health_status
            status.health_score = result.health_score
            status.last_health_check = result.timestamp
            return result

    def check_all_backends_health(self) -> dict[str, HealthCheckResult]:
        """Check health of all available backends."""
        results = {}
        with self._lock:
            available = [
                (name, status.adapter)
                for name, status in self._statuses.items()
                if status.available and status.adapter
            ]
        for name, adapter in available:
            results[name] = self.check_backend_health(name)
        return results
    ...
    def generate_comparison_matrix(self) -> BackendComparisonMatrix:
        """Generate comparison matrix for all backends."""
        return self._comparison_matrix.generate(self._statuses)

    def get_comparison_dict(self) -> dict:
        """Get comparison data as dictionary."""
    ...
    def get_feature_matrix(self) -> dict[str, dict[str, bool]]:
        """Get feature matrix for all backends."""
        matrix = self.generate_comparison_matrix()
        return matrix.feature_matrix()
    ...
    def recommend_backend(
        ...
        """Recommend best backend for requirements."""
        matrix = self.generate_comparison_matrix()
        return matrix.recommend_backend(qubit_count, needs_gpu, needs_noise)
    ...
    def save_performance_history(self, filepath: str | None = None) -> None:
        """Save performance history to file."""
        self._performance_tracker.save_to_file(filepath)

    # ==================== UTILITY METHODS ====================

    def get_gpu_backends(self) -> list[str]:
        """Get list of GPU-accelerated backends."""
        gpu_backends = []
        with self._lock:
            for name, status in self._statuses.items():
                if status.available and status.capabilities:
                    if getattr(status.capabilities, "gpu_acceleration", False):
                        gpu_backends.append(name)
        return gpu_backends
    ...
    def get_best_backend_for_circuit(
        ...
        """Get best backend for a circuit based on requirements."""
        recommended = self.recommend_backend(qubit_count, needs_gpu, needs_noise)
        if recommended:
            return recommended
        ...
        if not available:
            return None
        if needs_gpu:
            gpu_backends = self.get_gpu_backends()
            if gpu_backends:
...
def get_registry() -> BackendRegistry:
    """Get the default backend registry (singleton)."""
...
def reset_registry() -> None:
    ...
    with _registry_lock:
        if _default_registry is not None:
            _default_registry.stop_health_monitoring()
        _default_registry = None


def get_backend(name: str) -> BaseBackendAdapter:
    """Convenience function to get a backend adapter."""
...
def is_backend_available(name: str) -> bool:
    """Convenience function to check if a backend is available."""
    return get_registry().is_available(name)
