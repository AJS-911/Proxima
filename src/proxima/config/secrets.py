            ...
            try:
                ...
                if meta_value:
                    ...
                    metadata = SecretMetadata(
                        ...
                        updated_at=datetime.fromisoformat(meta_dict.get("updated_at", datetime.now().isoformat())),
                    )
            except Exception:
                pass
...
class EncryptedFileStorage(SecretStorage):
    ...
    def __init__(self, storage_path: Path | None = None, encryption_key: bytes | None = None) -> None:
    ...
    def retrieve(self, key: str) -> SecretResult:
        try:
            ...
            metadata = SecretMetadata(
                ...
                updated_at=datetime.fromisoformat(entry.get("updated_at", datetime.now().isoformat())),
            )
...
class SecretManager:
    ...
    def retrieve(self, key: str, backend: SecretBackend | None = None) -> SecretResult:
        ...
        for backend_type in [SecretBackend.KEYRING, SecretBackend.ENCRYPTED, SecretBackend.ENVIRONMENT]:
...
def validate_api_key_format(key: str, provider: str) -> tuple[bool, str]:
    ...
    validations = {
        ...
        "anthropic": (lambda k: k.startswith("sk-ant-") and len(k) > 30, "Anthropic keys start with 'sk-ant-'"),
    }
