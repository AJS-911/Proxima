...
def validate_verbosity(value: Any, path: str, result: ValidationResult) -> None:
    ...
    if not isinstance(value, str):
        result.add_error(
            path, "Verbosity must be a string",
            current_value=value, expected_type="str"
        )
    elif value.lower() not in valid_values:
        result.add_error(
            path, f"Invalid verbosity level: {value}",
...
def validate_output_format(value: Any, path: str, result: ValidationResult) -> None:
    ...
    if not isinstance(value, str):
        result.add_error(
            path, "Output format must be a string",
            current_value=value, expected_type="str"
        )
    elif value.lower() not in valid_formats:
        result.add_error(
            path, f"Invalid output format: {value}",
...
def validate_backend(value: Any, path: str, result: ValidationResult) -> None:
    ...
    if not isinstance(value, str):
        result.add_error(
            path, "Backend must be a string",
            current_value=value, expected_type="str"
        )
    elif value.lower() not in valid_backends:
        result.add_warning(
            path, f"Unknown backend: {value}",
...
def validate_timeout(value: Any, path: str, result: ValidationResult) -> None:
    ...
    if not isinstance(value, (int, float)):
        result.add_error(
            path, "Timeout must be a number",
            current_value=value, expected_type="int or float"
        )
    elif value <= 0:
        result.add_error(
            path, "Timeout must be positive",
            suggestion="Set a value greater than 0 seconds",
            current_value=value
        )
    elif value > 86400:  # 24 hours
        result.add_warning(
            path, "Timeout is very large (>24 hours)",
...
def validate_llm_provider(value: Any, path: str, result: ValidationResult) -> None:
    ...
    if not isinstance(value, str):
        result.add_error(
            path, "LLM provider must be a string",
            current_value=value, expected_type="str"
        )
    elif value.lower() not in valid_providers:
        result.add_warning(
            path, f"Unknown LLM provider: {value}",
...
def validate_url(value: Any, path: str, result: ValidationResult) -> None:
    ...
    try:
        ...
        if not parsed.scheme:
            result.add_error(
                path, "URL missing scheme (http:// or https://)",
                suggestion=f"Add scheme: https://{value}",
                current_value=value
            )
        elif parsed.scheme not in ("http", "https"):
            result.add_warning(
                path, f"Unusual URL scheme: {parsed.scheme}",
                suggestion="Standard schemes are http:// or https://",
                current_value=value
            )
        if not parsed.netloc:
            result.add_error(
                path, "URL missing host",
                current_value=value
            )
    except Exception:
        result.add_error(
            path, "Invalid URL format",
            current_value=value
        )
...
def validate_memory_threshold(value: Any, path: str, result: ValidationResult) -> None:
    ...
    if not isinstance(value, int):
        result.add_error(
            path, "Memory threshold must be an integer",
            current_value=value, expected_type="int"
        )
    elif value < 128:
        result.add_warning(
            path, "Memory threshold is very low (<128 MB)",
            suggestion="This may cause frequent warnings on modern systems",
            current_value=value
        )
    elif value > 65536:  # 64 GB
        result.add_warning(
            path, "Memory threshold is very high (>64 GB)",
...
def validate_storage_backend(value: Any, path: str, result: ValidationResult) -> None:
    ...
    if not isinstance(value, str):
        result.add_error(
            path, "Storage backend must be a string",
            current_value=value, expected_type="str"
        )
    elif value.lower() not in valid_backends:
        result.add_error(
            path, f"Invalid storage backend: {value}",
...
def validate_model_name(value: Any, path: str, result: ValidationResult) -> None:
    ...
    if not isinstance(value, str):
        result.add_error(
            path, "Model name must be a string",
            current_value=value, expected_type="str"
        )
        return
    ...
    if not any(re.match(p, value, re.IGNORECASE) for p in common_patterns):
        result.add_info(
            path, f"Model name '{value}' doesn't match common patterns",
...
def validate_env_var_name(value: Any, path: str, result: ValidationResult) -> None:
    ...
    if not isinstance(value, str):
        result.add_error(
            path, "Environment variable name must be a string",
            current_value=value, expected_type="str"
        )
        return
    ...
    if not re.match(r"^[A-Z_][A-Z0-9_]*$", value):
        result.add_warning(
            path, "Environment variable name doesn't follow conventions",
...
def validate_settings(config: dict[str, Any]) -> ValidationResult:
    ...
    if isinstance(general, dict):
        ...
        validate_output_format(general.get("output_format", "text"), "general.output_format", result)
        validate_path(general.get("data_dir", ""), "general.data_dir", result)
        validate_storage_backend(general.get("storage_backend", "sqlite"), "general.storage_backend", result)
    ...
    if isinstance(backends, dict):
        validate_backend(backends.get("default_backend", "auto"), "backends.default_backend", result)
    ...
    if isinstance(resources, dict):
        validate_memory_threshold(
            ...
            "resources.memory_warn_threshold_mb", result
        )
        validate_memory_threshold(
            resources.get("memory_critical_threshold_mb", 8192),
            "resources.memory_critical_threshold_mb", result
        )
        validate_timeout(
            resources.get("max_execution_time_seconds", 3600),
            "resources.max_execution_time_seconds", result
        )
...
def validate_config_file(path: Path) -> ValidationResult:
    ...
    if not path.exists():
        result.add_info(
            str(path), "Configuration file not found",
    ...
    try:
        ...
        result.add_error(
            str(path), f"Invalid YAML syntax: {e}",
    ...
    if not isinstance(config, dict):
        result.add_error(
            str(path), "Configuration must be a YAML dictionary/mapping",
