...
class LocalLLMDetector:
	...
	def __init__(self, timeout_s: float = 0.35):
		...
		self._model_files_cache: Optional[Dict[str, list]] = None
	...
	def check_ollama(self, configured_endpoint: str = "", force: bool = False) -> Optional[str]:
	...
	def check_lm_studio(self, configured_endpoint: str = "", force: bool = False) -> Optional[str]:
	...
	def check_llama_cpp(self, configured_endpoint: str = "", force: bool = False) -> Optional[str]:
	...
	def detect_all_parallel(self, force: bool = False) -> Dict[str, Optional[str]]:
		...
		def check_provider(provider: str) -> Tuple[str, Optional[str]]:
...
class APIKeyManager:
	...
	def __init__(self, settings: Settings, storage_path: Optional[str] = None):
	...
	def _load_from_storage(self, provider_name: str) -> Optional[str]:
...
class ConsentGate:
	...
	def __init__(
		...
		self._persistent_consents: Optional[Dict[str, bool]] = None

	def check_consent(self, provider: LLMProvider) -> Optional[bool]:
		...
		auto_allow = (
			self.settings.consent.auto_approve_local_llm if is_local 
