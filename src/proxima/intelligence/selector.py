    ...
    def from_circuit(cls, circuit: "Circuit") -> "CircuitCharacteristics":
        ...
        for gate in circuit.gates:
            ...
            if gate.name.lower() not in (
                "h", "x", "y", "z", "s", "t", "cx", "cnot", "cz",
                "swap", "rx", "ry", "rz", "u1", "u2", "u3", "i",
                "id", "identity", "measure", "m", "barrier"
            ):
                custom = True
...
class BackendRegistry:
    ...
    def _register_default_backends(self) -> None:
        ...
        self.register(BackendCapabilities(
            name="numpy",
    ...
    def _score_compatibility(
        ...
        if backend.supported_gate_types:
            if characteristics.gate_types:
                overlap = len(
                    characteristics.gate_types & backend.supported_gate_types
                )
...
class BackendSelector:
    ...
    def select(
        ...
        if (
            strategy == SelectionStrategy.LLM_ASSISTED
            and self._llm_recommender is not None
        ):
            try:
