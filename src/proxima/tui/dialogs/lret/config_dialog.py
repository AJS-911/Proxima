...
class LRETConfigDialog(ModalScreen):
    ...
    LRETConfigDialog .not-available-message {
        padding: 2;
        color: $warning;
        text-align: center;
    }
    ...
    LRETConfigDialog Input {
        width: 100%;
        height: 3;
    }
    
    LRETConfigDialog Select {
        width: 100%;
        height: 3;
    }
    ...
    def compose(self) -> ComposeResult:
        with Vertical(classes="dialog-container"):
            ...
            try:
                ...
                yield Static(
                    f"⚠️ Error loading LRET configuration: {e}",
                    classes="not-available-message"
                )
                with Horizontal(classes="dialog-footer"):
                    yield Button("Close", variant="default", id="cancel-btn")
    ...
    def _compose_general_settings(self) -> ComposeResult:
        ...
        if not self._config:
            yield Static("No configuration loaded")
            return
