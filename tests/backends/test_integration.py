...
class TestBackendRegistryIntegration:
    ...
    def test_all_backends_registered(self, mock_all_backends):
        ...
        with patch.object(BackendRegistry, '_discover_backends', return_value=mock_all_backends):
...
class TestMultiBackendComparison:
    ...
    def test_same_circuit_multiple_backends(self, mock_all_backends, bell_state_circuit):
        """Test running same circuit on multiple backends."""
        from proxima.data.compare import BackendComparator
        
        with patch("proxima.backends.registry.BackendRegistry._discover_backends", return_value=mock_all_backends):
    ...
    def test_comparison_with_identical_parameters(self, mock_all_backends, bell_state_circuit):
    ...
    def test_result_accuracy_comparison(self, mock_all_backends, bell_state_circuit):
        ...
        mock_all_backends["cirq"].execute.return_value.data = {"counts": {"00": 498, "11": 502}}
        mock_all_backends["qiskit"].execute.return_value.data = {"counts": {"00": 510, "11": 490}}
...
class TestFallbackLogicIntegration:
    ...
    def test_unsupported_feature_fallback(self, mock_all_backends):
        ...
        mock_all_backends["cuquantum"].validate_circuit.return_value.message = "DM not supported"
    ...
    def test_resource_limit_fallback(self, mock_all_backends):
        ...
        large_circuit = {"num_qubits": 25, "gates": [], "simulator_type": "state_vector"}
        
        # lret has lower qubit limit
        mock_all_backends["lret"].validate_circuit.return_value.valid = False
        mock_all_backends["lret"].validate_circuit.return_value.message = "Exceeds qubit limit"
...
class TestConfigurationIntegration:
    ...
    def test_config_affects_backend_selection(self, mock_all_backends, sample_config, bell_state_circuit):
    ...
    def test_timeout_configuration(self, mock_all_backends, bell_state_circuit, sample_config):
        """Test timeout configuration is respected."""
        from proxima.core.executor import CircuitExecutor
        
        with patch("proxima.backends.registry.BackendRegistry._discover_backends", return_value=mock_all_backends):
            executor = CircuitExecutor(timeout_seconds=sample_config["backends"]["timeout_s"])
    ...
    def test_max_qubits_configuration(self, mock_all_backends, sample_config):
        ...
        with patch("proxima.backends.registry.BackendRegistry._discover_backends", return_value=mock_all_backends):
            selector = BackendSelector(max_qubits=sample_config["backends"]["max_qubits"])
    ...
    def test_fallback_backend_configuration(self, mock_all_backends, bell_state_circuit, sample_config):
