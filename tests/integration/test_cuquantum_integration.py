...
def mock_gpu_environment(mock_gpu_device_info):
    ...
    with patch("proxima.backends.cuquantum_adapter.CuQuantumAdapter._check_cuda_available") as mock_cuda:
        with patch("proxima.backends.cuquantum_adapter.CuQuantumAdapter._detect_gpu_devices") as mock_devices:
            with patch("proxima.backends.cuquantum_adapter.CuQuantumAdapter._check_cuquantum_available") as mock_cuq:
                with patch("proxima.backends.cuquantum_adapter.CuQuantumAdapter._check_qiskit_gpu_available") as mock_qiskit:
...
class TestGPUAvailability:
    ...
    def test_adapter_initialization_without_gpu(self):
        ...
        with patch.object(CuQuantumAdapter, "_check_cuda_available", return_value=False):
            adapter = CuQuantumAdapter()
...
class TestMemoryEstimation:
    ...
    def test_memory_estimate_large_circuit(self, memory_manager):
        ...
        assert abs(estimate.state_vector_mb - expected_sv_mb) < 1.0  # Allow 1 MB tolerance
    ...
    def test_memory_estimate_with_single_precision(self, memory_manager):
        ...
        assert abs(single_estimate.state_vector_mb - double_estimate.state_vector_mb / 2) < 0.1
...
class TestFallbackStrategy:
    ...
    def test_no_fallback_when_forced(self, memory_manager):
        ...
        should_fallback, reason = memory_manager.should_fallback_to_cpu(5, force_gpu=True)
...
class TestPerformance:
    ...
    def test_gpu_vs_cpu_speedup(self, sample_circuit):
        ...
        cpu_result = cpu_sim.run(sample_circuit, shots=1000).result()
    ...
    def test_measurement_distribution(self, sample_circuit):
        ...
        if adapter.is_available():
            ...
            for key in set(cpu_counts.keys()) | set(gpu_counts.keys()):
                ...
                assert abs(cpu_prob - gpu_prob) < 0.05, f"Distribution mismatch for {key}"
