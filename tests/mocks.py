...
class MockDataFactory:
    ...
    def quantum_counts(
        ...
        counts = {}
        remaining = shots
        for state, prob in zip(states[:-1], probs[:-1]):
...
class MockBackendRegistry:
    ...
    def get(self, name: str) -> Optional[MockQuantumBackend]:
...
class MockPsutil:
    ...
    def __init__(self, resources: Optional[MockSystemResources] = None):
...
@contextmanager
def mock_psutil(resources: Optional[MockSystemResources] = None) -> Generator[MockPsutil, None, None]:
    """
...
class MockConsentManager:
    ...
    def __init__(
        ...
        require_consent_for: Optional[List[str]] = None,
    ):
